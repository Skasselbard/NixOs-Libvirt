...
with lib;
{
  ...
  options = {
    libvirt.domains = mkOption{
      type = types.attrsOf  (types.submodule ((import ./domain.nix).domain_options{inherit lib types;}));
  ...
  config = let
    domains = (import ./domain.nix).gen_domain_xml{inherit lib; domains = config.libvirt.domains;};
  in {
    ...
    systemd.services = lib.mapAttrs' (name: domain: lib.nameValuePair "libvirtd-domain-${name}" {
      path = with pkgs; [nixos-generators qemu libguestfs];
      environment = { NIX_PATH = config.nix_path; } ;
      preStart =  concatStrings(builtins.attrValues(attrsets.mapAttrs(disk_name: disk: 
        if disk.type == "nix" then (import ./scripts.nix).qcowFromNix{ inherit disk; disk_name = "${name}-${disk_name}";}
        else "") domain.disks));
